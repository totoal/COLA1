...
Modignore=data.field('Module_ignore') ##this is a manual column where you can keep track of which Modules should be ignored (for example if one module is contaminated)
	...
	for module in ['A','B']:
		...
		for i in range(3):
			try:
				...
				pyplot.savefig(SAVE_FOLDER+'%s_ID_%s_line_%s_mod%s.png'%(FIELD,thisID,int(thisline),module))
				pyplot.clf()
...
sel_AB=(flux_5008_A>-1E9)*(flux_5008_B>-1E9)*~(flux_5008_B==0.)*~(flux_5008_A==0.)
